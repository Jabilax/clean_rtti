...
#define equal_string(compare) [](const std::string & word) -> bool { return word == compare; }
...
namespace
{
    ...
    void create_flows()
    {
        // ------------------------------------------- enum flow ----------------------------------------------
        flow enum_flow;
        compare_node(A, equal_string("enum"));
        execute_node(B, is_name, create_enum);
        compexe_node(C, equal_string(";"), remove_enum);
        execute_node(D, equal_string("{"), do_nothing);
        execute_node(F, is_name, add_enum_name);
        compare_node(G, equal_string("="));
        compare_node(H, equal_string(","));
        execute_node(I, equal_string("}"), do_nothing);
        execute_node(J, expect_anything, do_nothing);
        enum_flow.start = &A;
        A.true_node = &B;
        B.next_node = &C;
        C.false_node = &D;
